function onlyNumberLetters(password) {
...
function checksNumber(password) {
    return (
        /\d/.test(password) &&
        /[.,:;!?]/.test(password) &&
...
function passwordErrorHtml(password) {
    ...
    if (onlyNumberLetters(password) === false) {
        error.push(
            '<li id="error2">Password can only contain letter and number.</li>'
        );
    }
    if (checksNumber(password) === false) {
        error.push(
...
//         error.push(
//             '<li id="error2">Username must be between 8 to 13 characters</li>'
//         );
//     }
